...
class Storage {
  ...
  Future<void> uploadFile(
    ...
    try {
      await storage.ref('mids/$fileName').putFile(file);
  ...
  Future<firebase_storage.ListResult> listFiles() async {
    firebase_storage.ListResult results = await storage.ref('mids').listAll();

    results.items.forEach((firebase_storage.Reference ref) {
      print('Found Mids file: $ref');
    });
  ...
  Future<String> downloadURL(String pdfName) async {
    ...
    print("pname=$pname");
    String downloadURL =
        (await storage.ref('mids/$pdfName').getDownloadURL())!.toString();
      ...
      body: _loading
          ? Column(
              children: [
                ...
                const Center(
                  child: Text(
                      "Fact: Your eyes look in different directions when reading...",
                      style: TextStyle(
                          color: Colors.black,
